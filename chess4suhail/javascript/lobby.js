...
var lobby = {
  ...
  updateGameList: function(gameList) {
    ...
    jQuery.each(gameList, function(index, game) {
        content += "<tr><td>" + game.creator + "</td><td>" +
          ...
          (game.time_limit ?  (game.time_limit + " min") : "بدون توقيت") +
           "</td><td>";
  ...
  updatePlayerList: function(playerList) {
    ...
    $("#lobbyStatus").html("ساحة اللعب<br/>" +
        (playerList.length == 1 ?
         "لاعب واحد " : "" + playerList.length + " لاعبين ") +
         " في الساحة ");
    ...
    if (lobby.playerMap) {
      ...
      for (oldPlayer in lobby.playerMap) {
        if (!playerMap[oldPlayer]) {
          chatStr += oldPlayer + " ترك الساحة<br/>";
        }
      }
      for (newPlayer in playerMap) {
        if (!lobby.playerMap[newPlayer]) {
          chatStr += newPlayer + " دخل على الساحة<br/>";
        }
      }
  ...
  validateInvite: function(form) {
    ...
    $(form).find("#email").each(function(index) {
        ...
        if (!this.value.match(regexp)) {
          err = "طج أيميل رفيقك هون. مامنقبل غير إيميلات غوغل, و مافي فراطة مشان الترجيع";
        } else if (this.value.toLowerCase() == blitz.user_email.toLowerCase()) {
          err = "عم تستهبل علينا؟ مافيك تدعو حالك لتلعب مع حالك";
        }
        if (err) {
          success = false;
          $(form).
